...
const EXPAND_PROMPT = `你是一位专业的青少年科普作家。你的任务是根据标题和摘要，撰写一篇完整的科普文章。

## 规则
1. 字数要求：1500-2000字
2. 面向小升初学生（11-12岁），语言通俗易懂，有轻思辨性
3. 围绕标题主题展开，补充科学原理、背景知识、具体案例、数据支撑
4. 文风客观平实，不空洞说教，不官话套话
5. 段落清晰，适合碎片时间阅读
6. 可使用<h2>、<h3>小标题组织内容

## 输出要求
输出完整正文HTML。使用<p>标签包裹段落，可适当使用<h2>、<h3>、<blockquote>等语义化标签。
不要输出标题、不要输出任何元信息，只输出正文。`;

const COMPRESS_PROMPT = `你是一位专业的青少年科普作家。你的任务是将较长文章精简到1000-1500字：

## 规则
1. 目标字数：1000-1500字（约4-5分钟阅读量）
...
function buildExpandPrompt(article) {
  return `请根据以下标题和摘要，撰写一篇1500-2000字的完整科普文章：

标题：${article.title}
来源：${article.source || '未知'}
摘要：${article.summary || article.content || '无'}

要求：
- 围绕标题主题，补充详细的科学原理、背景知识、具体案例
- 1500-2000字，内容充实饱满
- 面向11-12岁学生，通俗易懂
- 只输出正文HTML`;
}
...
function buildCompressPrompt(article) {
  const bodyContent = article.content || article.summary || '';
  const charCount = (bodyContent || '').replace(/<[^>]*>/g, '').replace(/\s/g, '').length;
  return `请将以下较长文章精简到1000-1500字：
...
精简要求：
- 目标1000-1500字，不得低于1000字
- 保留核心知识点、数据、案例
- 保留所有<img>标签，不要删除任何图片
- 只输出正文HTML`;
}

function buildQuizPrompt(article) {
  const plainText = (article.content || '').replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
  const charCount = plainText.length;
  const readMinutes = Math.ceil(charCount / 300);

  let singleCount, judgeCount;
  if (readMinutes <= 5) {
    singleCount = 3; judgeCount = 3;
  } else if (readMinutes <= 10) {
    singleCount = 4; judgeCount = 4;
  } else {
    singleCount = 5; judgeCount = 5;
  }

  const paragraphs = plainText.split(/[。！？；\n]+/).filter(s => s.trim().length > 10);
  const numberedText = paragraphs.map((s, i) => `[${i + 1}] ${s.trim()}`).join('\n');

  const prompt = `你是一位专业的青少年科普测验出题专家。请根据以下文章内容生成知识点测验题目。

## 重要规则
- 所有题目答案必须严格来自下方提供的文章正文，不得引入文章以外的知识
- sourceText必须是下方原文中的完整原句，一字不改
- sourceParagraph必须是对应段落的正确编号
- 学生需要能在文章中找到每道题的答案
...
## 出题要求
1. 生成 ${singleCount} 道单项选择题 + ${judgeCount} 道正误判断题
2. 考察范围：关键数据、科学原理、事物分类、因果逻辑、客观事实
3. 每道题必须标注对应的原文段落编号和关键佐证句子（原文原句）
  ...
  "single": [
    {
      ...
      "correctOption": "正确选项完整文本",
      "sourceParagraph": 1,
      "sourceText": "支撑本题答案的原文关键句子（必须是原文原句）"
    }
  ],
  "judge": [
    {
      ...
      "correctAnswer": true,
      "sourceParagraph": 2,
      "sourceText": "支撑该判断的原文关键句子（必须是原文原句）"
    }
  ]
}
\`\`\``;

  return { prompt, singleCount, judgeCount };
}
...
function parseQuizResponse(text) {
  try {
    ...
    if (!quiz.single || !quiz.judge) return null;
    if (!Array.isArray(quiz.single) || !Array.isArray(quiz.judge)) return null;
    return quiz;
  } catch {
    return null;
  }
