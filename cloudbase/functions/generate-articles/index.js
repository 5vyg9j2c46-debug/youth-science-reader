...
const DEEPSEEK_KEY = process.env.AI_API_KEY || 'sk-86749520e7c44335a60ccd5bc0e739a8';
...
exports.main = async (event, context) => {
  ...
  const isExtra = event.extra === true;
  ...
  const tcb = context.app ? context.app.database() : null;
  if (tcb) {
    try {
      const coll = tcb.collection('articles');
      await coll.where({ date: dateStr }).remove();
      await coll.add(output);
      console.log(`Saved to database: ${dateStr}`);
    } catch (e) {
      console.warn(`DB save failed: ${e.message}`);
    }
  }

  const filePath = path.join('/tmp', `${dateStr}.json`);
  ...
  return {
    ...
    articles: articles.map(a => ({ id: a.id, title: a.title, category: a.category, hasQuiz: !!a.quiz }))
  };
};
